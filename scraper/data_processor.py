...
_PM_JSON_PATH = os.path.join(os.path.dirname(__file__), 'passmark_scores.json')


def fetch_passmark_scores():
    """Load PassMark scores from passmark_scores.json (generated by fetch_passmark.js).
    Falls back to scraping HTML list pages when the JSON file is absent.
    CPU HTML scraping covers Intel only — AMD rows are JS-rendered.
    Returns {'CPU': {name: score}, 'GPU': {name: score}}. Never raises."""
    # Prefer pre-generated JSON (includes AMD CPUs via Puppeteer)
    if os.path.exists(_PM_JSON_PATH):
        try:
            with open(_PM_JSON_PATH, encoding='utf-8') as f:
                data = json.load(f)
            cpu_n = len(data.get('CPU', {}))
            gpu_n = len(data.get('GPU', {}))
            print(f"  PassMark loaded from JSON: CPU={cpu_n}, GPU={gpu_n} "
                  f"(generated {data.get('generated', 'unknown')})")
            return {'CPU': data.get('CPU', {}), 'GPU': data.get('GPU', {})}
        except Exception as exc:
            print(f"  passmark_scores.json unreadable ({exc}), falling back to HTTP scrape.")

    # HTTP fallback — Intel CPUs + all GPUs
    scores = {'CPU': {}, 'GPU': {}}
        ...
        try:
            ...
            for match in _PM_ROW_RE.finditer(content):
                ...
                try:
                    ...
                    continue
            print(f"  PassMark {category} (HTTP): {len(scores[category])} scores loaded.")
        except Exception as exc:
